...
def minPickDP(total:int, coinSet:Ints) -> int:
   """ Dynamic Programming Coin Change
   Returns min number of value picked from valueSet that = total
   """
   res = [0 for _ in range(total+1)]
   for val in range(1, total+1):
      res[val] = sys.maxsize # to be replaced in if below if condition is true
      for coin in coinSet:
         if val >= coin and res[val - coin] + 1 < res[val]:
            res[val] = res[val-coin] + 1
   # print('res', res)
...
def LCSBackTrack(v:str, w:str, penalty=0, method='', matchScore=1) -> list: #[StrMat, int, int, int]:
   ...
   scoreLU = Blosum62 if method=='global' else Pam250 if method=='local' else dict() #ScoreMat()
   useLU = len(scoreLU) > 0
...
def topologicalSort(graph:WtGraph) -> WPaths:
   ...
   while len(list(unvisited.keys())) - len(toDel) > 0:
      cyclic = True
      unvisitedItems = [o for o in unvisited.items() if o[0] not in toDel]
      for src, edges in unvisitedItems:#.items() if src not in toDel else []:
         # src in loop from Eg.: 0, 1, 2, 0, 1, 0
         cyclicable = any([dst in unvisitedItems for dst, _ in edges]) 
         if not cyclicable:
            # when:
